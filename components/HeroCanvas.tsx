...
const PostProcessing = ({
  ...
  const render = useMemo(() => {
    ...
    // Neon purple scan line instead of red
    const purpleOverlay = vec3(0.486, 0.227, 0.929).mul(oneMinus(scanLine)).mul(0.4);
...
const Scene = () => {
  ...
  const { material, uniforms } = useMemo(() => {
    ...
    // Purple dot grid instead of red
    const mask = dot.mul(flow).mul(vec3(0.486, 0.227, 0.929));
    ...
    <div className="h-svh relative">
      <div className="h-svh uppercase items-center w-full absolute z-40 pointer-events-none px-10 flex justify-center flex-col">
        <div className="text-4xl md:text-6xl xl:text-7xl 2xl:text-8xl font-extrabold font-heading">
          <div className="flex space-x-3 lg:space-x-6 overflow-hidden text-off-white">
        ...
        <div className="text-xs md:text-lg xl:text-xl 2xl:text-2xl mt-3 overflow-hidden text-silver font-heading font-medium tracking-[0.15em]">
          <div
        ...
        <div className="text-[10px] md:text-sm xl:text-base mt-4 overflow-hidden">
          <div
          ...
          >
            <span className="px-4 py-1.5 border border-neon/30 rounded-full text-neon-bright font-heading tracking-[0.2em] text-[10px] md:text-xs uppercase">
              {tagline}
            </span>
          </div>
        </div>
